...
def ged261_dynamic_filter_and_agg(feat, dataset_path, name, outcome="best", **filters):
    """Filter UCDP GED v26.1 by spatial and attribute criteria, then aggregate.
    ...
    Args:
        outcome: Key of the field to aggregate (e.g. "best", "deaths_a").
    """
    # Separate aggregate from non-aggregate filters
    agg_filters = {}
    non_agg_filters = {}  # key -> list of selected values
    for key, value in filters.items():
        ...
        if value.get("aggregate", True):
            ...
            # Non-aggregate: each selected value becomes a separate output
            if value["type"] == "categorical":
                non_agg_filters[key] = value.get("selected", [])
            elif value["type"] == "range":
                # Range with aggregate=False is unusual; treat as a single aggregate filter
                agg_filters[key] = value
    ...
    if not non_agg_filters:
        # No disaggregation needed — single result
