...
if mode == "all":
    ...
    requests = Request.objects.filter(
        status=1,
        submit_time__gt=1486443600,
        contact__regex=r"^(?!.*wm.edu)(?!.*aiddata.org)",
    )
else:
...
dataset_df = pd.DataFrame(
    {"name": dataset_values, "requests": dataset_counts}
).sort_values(by="requests", ascending=False)
...
request_df["month"] = request_df["datetime"].apply(
    lambda z: "{0}{1}".format(z.year, str(z.month).zfill(2))
)
...
monthly_request_df = (
    request_df[["month", "requests"]].groupby(["month"], as_index=False).sum()
)
weekly_request_df = (
    request_df[["week", "requests"]].groupby(["week"], as_index=False).sum()
)
daily_request_df = (
    request_df[["day", "requests"]].groupby(["day"], as_index=False).sum()
)
...
user_df = request_df.groupby("contact", as_index=False).agg({"requests": "sum"})
...
monthly_user_df = (
    request_df[["month", "contact"]]
    .groupby(["month"], as_index=False)
    .agg({"contact": pd.Series.nunique})
)
monthly_user_df.columns = ["month", "users"]

weekly_user_df = (
    request_df[["week", "contact"]]
...
monthly_df["datetime"] = monthly_df["month"].apply(
    lambda z: datetime.datetime.strptime(z, "%Y%m")
)
...
weekly_df["datetime"] = weekly_df["week"].apply(
    lambda z: datetime.datetime.strptime(z, "%Y%m%d")
)
...
daily_df["datetime"] = daily_df["day"].apply(
    lambda z: datetime.datetime.strptime(z, "%Y%m%d")
)
...
monthly_plot_df = (
    monthly_df.loc[
        (monthly_df.datetime >= min_datetime) & (monthly_df.datetime <= max_datetime)
    ]
    .copy(deep=True)
    .reset_index()
)
weekly_plot_df = (
    weekly_df.loc[
        (weekly_df.datetime >= min_datetime) & (weekly_df.datetime <= max_datetime)
    ]
    .copy(deep=True)
    .reset_index()
)
daily_plot_df = (
    daily_df.loc[
        (daily_df.datetime >= min_datetime) & (daily_df.datetime <= max_datetime)
    ]
    .copy(deep=True)
    .reset_index()
)
...
plt.plot(
    "month", "users", data=monthly_plot_df, linewidth=1, color="skyblue", label="users"
)
plt.plot(
    "month",
    "requests",
...
monthly_plot_path = os.path.join(
    output_path, "{}_geoquery_monthly_plot.png".format(mode)
)
...
plt.figure(figsize=(6, 3))
plt.plot(
    "week", "users", data=weekly_plot_df, linewidth=1, color="skyblue", label="users"
)
plt.plot(
    "week", "requests", data=weekly_plot_df, linewidth=1, color="red", label="requests"
)
plt.legend()
plt.xticks(rotation=60)
plt.tight_layout()
...
plt.plot(
    "day", "users", data=daily_plot_df, linewidth=1, color="skyblue", label="users"
)
plt.plot(
    "day", "requests", data=daily_plot_df, linewidth=1, color="red", label="requests"
)
plt.legend()
plt.xticks(rotation=60)
plt.tight_layout()
...
for ix, step_ts in enumerate(repeat_df["month_ts"]):
    ...
    # ])
    agg = (
        Request.objects.filter(submit_time__gt=start, complete_time__lt=step_timestamp)
        .values("contact")
        .annotate(count=Count("id"))
        .filter(count__gt=min_requests)
    )
    n_repeat = int(len(list(agg)))
    repeat_df.loc[ix, "count"] = n_repeat
    if ix > 0:
        repeat_df.loc[ix, "gain"] = n_repeat - repeat_df.loc[ix - 1, "count"]
    total_users = Request.objects.filter(
        submit_time__gt=start, complete_time__lt=step_timestamp
