...
@shared_task
def dispatch_pending_extract_tasks(limit=1000):
    """Find pending extract tasks and dispatch each as a Celery task.
...
@shared_task
def free_stale_extract_tasks(minutes=30):
    """Reset tasks stuck in 'locked' (status=2) state back to pending.
