...
3. Here is an example command for installing the helm chart from the parent directory.
   ...
   ```shell
   helm install --create-namespace --namespace geoquery --values my-values.yaml geoquery-release ./helm_chart
   ```
   - `--namespace geoquery` sets which namespace to install the chart in
   ...
   - `geoquery-release` is the name of the release (installed instance of this chart). You will use this name to upgrade or uninstall this release in the future.
...
helm list
# change "geoquery-release" to be the correct release name
helm upgrade geoquery-release ./helm_chart
```
