...
7. Set "geoquery" as our default namespace using `kubectl`.
...
10. Install the helm chart into the local cluster.
   If there is already a namespace called "geoquery", remove `--create-namespace`
   ```shell
   # cd to the root of this repository
   cd ..
   helm install --create-namespace --namespace geoquery geoquery-release ./helm_chart -f values.yaml
   ```
