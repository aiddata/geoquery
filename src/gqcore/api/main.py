...
async def root():
    return [
        "/datasets", "/get_dataset/{dataset_id}", "/dataset_resources/{dataset_id}",
        "/feature_collections", "/feature_collections/{fc_id}", "/feat_map/{fc_id}", "/features/{feature_id}"
    ]
...
async def get_dataset(dataset_id: int):
    with get_conn() as conn:
        with conn.cursor() as cur:
            x = cur.execute("""SELECT * FROM datasets WHERE id = %s""", (dataset_id,)).fetchone()
            return x
...
async def get_dataset_resources(dataset_id: int):
    with get_conn() as conn:
        with conn.cursor() as cur:
            x = cur.execute("""SELECT * FROM dataset_resources WHERE dataset_id = %s""", (dataset_id,)).fetchall()
            return x
...
async def get_feature_collection(feature_collection_id: int):
    with get_conn() as conn:
        with conn.cursor() as cur:
            x = cur.execute("""SELECT * FROM feature_collections WHERE id = %s""", (feature_collection_id,)).fetchone()
            return x
...
async def get_feat_map(fc_id: int):
    with get_conn() as conn:
        with conn.cursor() as cur:
            x = cur.execute("""SELECT * FROM feat_map WHERE fc_id = %s""", (fc_id,)).fetchall()
            return x
...
async def get_feature(feature_id: int):
    with get_conn() as conn:
        with conn.cursor() as cur:
            x = cur.execute("""SELECT * FROM features WHERE id = %s""", (feature_id,)).fetchone()
            return x
...
async def get_coverage(feature_id: int):
    with get_conn() as conn:
        with conn.cursor() as cur:
            x = cur.execute("""SELECT * FROM coverage WHERE geom_id = %s AND status = 1""", (feature_id,)).fetchall()
            return x
...
async def get_coverage(dataset_id: int):
    with get_conn() as conn:
        with conn.cursor() as cur:
            x = cur.execute("""SELECT * FROM coverage WHERE dataset_id = %s AND status = 1""", (dataset_id,)).fetchall()
            return x
